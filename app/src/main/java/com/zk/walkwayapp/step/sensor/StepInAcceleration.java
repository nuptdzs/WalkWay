...
public class StepInAcceleration extends StepMode {
    ...
    public StepInAcceleration(Context context, StepCountListener stepCountListener) {
    ...
    public void onSensorChanged(SensorEvent event) {
        boolean flag = StepServiceFlagController.getStepServiceFlag(context);
        if(!flag){
            return;
        }
    ...
    private void preStep() {
        if (CountTimeState == 0) {
            ...
            if (stepCountListener != null) {
                stepCountListener.onStepCount(CURRENT_STEP);
            }
        }
    }
            ...
            TimerTask task = new TimerTask() {
                public void run() {
                    if (lastStep == CURRENT_STEP) {
                        ...
                        TEMP_STEP = 0;
                        //步行时间间隔
                        WALK_TIME_HOUR += nowTime - preTime;
                        WALK_TIME += nowTime - preTime;
                    } else {
