 ...
 * @author Maxwin 处理嵌套在scrollview内只显示一行问题
...
public class InScrollRefreshListView extends ListView implements OnScrollListener {
	 ...
	 * @param context
	 */
	public InScrollRefreshListView(Context context) {
		super(context);
		initWithContext(context);
	}

	public InScrollRefreshListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWithContext(context);
	}

	public InScrollRefreshListView(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		//根据模式计算每个child的高度和宽度，在AT_MOST模式下，前两位表示模式，所以右移两位
