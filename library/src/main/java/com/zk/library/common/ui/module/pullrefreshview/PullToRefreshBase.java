package com.zk.library.common.ui.module.pullrefreshview;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mMode == MODE_PULL_DOWN_TO_REFRESH || mMode == MODE_BOTH) {
			mHeaderLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseToRefreshLabel, pullToRefreshLabel, refreshingLabel, a, 0);
			addView(mHeaderLayout, 0, new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
			measureView(mHeaderLayout);
			mHeaderHeight = mHeaderLayout.getMeasuredHeight();
		}

		String pullToLoadLabel = context.getString(R.string.pull_to_load_pull_label);
		String loadingLabel = context.getString(R.string.pull_to_load_loading_label);
		String releaseToLoadLabel = context.getString(R.string.pull_to_load_release_label);
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
			mFooterLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseToLoadLabel, pullToLoadLabel, loadingLabel, a, 1);
			addView(mFooterLayout, new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
			mRefreshableView.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_ptrAdapterViewBackground, Color.WHITE));
		}
		a.recycle();
		a = null;
