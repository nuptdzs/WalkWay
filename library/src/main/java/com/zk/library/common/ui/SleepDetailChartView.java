...
public class SleepDetailChartView extends View {
    ...
    public SleepDetailChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public SleepDetailChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i <bars.size(); i++) {
            ...
            float top     = height - iHeight - barPadding ;
                ...
                if (text0 !=null || text0.length() > 0) {
                    if (text0.contains("\n")){
                        ...
                        for (int j =0; j < texts.length ; j++){
                            ...
                            canvas.drawText(texts[j], left - (textwidth - barWidth) / 2, barPadding + (fm0.bottom - fm0.top) * j, mTextPaint0);
                        }
                    }else {
                        float textWidth0 = mTextPaint0.measureText(text0);
                        canvas.drawText(text0, left - (textWidth0 - barWidth) / 2, barPadding/2, mTextPaint0);
                    }
                }
            ...
            if (bar.isShowBottomText()) {
                ...
                if (text2 !=null || text2.length() > 0) {
                    ...
                    canvas.drawText(bar.getBottomText(), left - (textWidth2 - barWidth) / 2, bottom + barPadding, mTextPaint1);
                }
            }
        }
    }
